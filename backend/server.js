...
// GET /user/:userId - Render dashboard with both user name and parsedResume data
app.get("/user/:userId", async (req, res) => {
  const { userId } = req.params;
  const { message, isSuccess } = req.query; // Get message from query params

  try {
    // Fetch both name and parsedResume fields
    const user = await User.findById(userId).select("name parsedResume");

    if (!user) {
      return res.status(404).json({ message: "User not found" });
    }

    // Render the EJS file with user name, parsedResume and optional message
    res.render("dashboard", {
      userName: user.name, // Direct user name from the main document
